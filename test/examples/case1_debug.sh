...
echo "âœ?Falco is running (PID: $(pgrep -x falco))"
echo ""
...
if [ "$NEW_LINES" -gt 0 ]; then
    echo "âœ“âœ“âœ?SUCCESS! Detected $NEW_LINES new log line(s)!"
    echo ""
    ...
    if sudo tail -n "$NEW_LINES" "$LOG_FILE" | grep -qi "sensitive file"; then
        echo "âœ“âœ“âœ?PERFECT! This is the expected 'Read sensitive file' rule!"
    else
        echo "âš?WARNING: Log detected but may not be the expected rule"
    ...
    echo "âœ?FAILED: No new logs detected after reading /etc/shadow"
    echo ""
    echo "Diagnosis:"
    echo "=========================================="
