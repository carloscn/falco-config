...
if [ "$NEW_LINES" -gt 0 ]; then
    ...
    if sudo tail -n "$NEW_LINES" "$LOG_FILE" | grep -qi "sensitive file\|read sensitive"; then
        echo "âœ“âœ“âœ?SUCCESS! Detected 'Read sensitive file' rule as expected!"
    elif sudo tail -n "$NEW_LINES" "$LOG_FILE" | grep -qi "Executing binary not part of base image"; then
        echo "âš?WARNING: Detected 'Executing binary not part of base image' rule instead"
        ...
        echo "âš?Detected logs but may not be the expected 'Read sensitive file' rule"
